...
FUNNEL_HEAD = [
    ...
    ("מהשלב הקודם", 20), ("מגיעים מהקודם", 14), ("חציון ימים מהקודם", 14),
    ("נמדד על", 11),
    ("מגיעים מההגשה", 14), ("חציון ימים מההגשה", 15), ("נמדד על ", 11),
    ("מהמגויסים עברו כאן", 16), ("נמדד על  ", 13),
]

FUNNEL_FMT = {2: "#,##0", 6: "0.0%", 7: "0.0", 8: "#,##0",
              9: "0.0%", 10: "0.0", 11: "#,##0", 12: "0.0%"}
...
def funnel_rows(ws, funnel, start=2):
    ...
    for row in funnel["rows"]:
        put(ws, r, [
            ...
            "" if not row["coverage"] or row["coverage"]["covered"] is None
            else f"{row['coverage']['covered_n']:,} מתוך "
                 f"{row['coverage']['covered_hires']:,} מגויסים",
...
def build_funnel_book(data):
    ...
    note(ws, r + 1, len(FUNNEL_HEAD), (
        ...
        "«מהמגויסים עברו כאן» היא העמודה החשובה ביותר, והיא אומרת משהו "
        "אחר לגמרי משיעור המעבר: איזה חלק מהמגויסים בכלל נרשמו בשלב "
        "הזה. שיעור המעבר אומר מה קורה למי שנמצא בשלב; הכיסוי אומר כמה "
        "מהגיוסים בכלל עוברים שם. שלב עם כיסוי נמוך אינו יושב מעל כל "
        "המשפך, וכמות שמוזנת בו אינה יכולה להסביר את סך הגיוס של "
        "הארגון - השאר מגיעים בדרך שאינה עוברת שם ואינה מתועדת "
        "בקבצים. הכיסוי נמדד רק על מגויסים שהחלון שלפניהם מכוסה "
        "בקבצים, כדי שתחילת הנתונים לא תיראה כמו כיסוי חסר.\n\n"
        "המדידה היא של *הגעה* לשלב, לא של *מעבר* בו. בקובץ אין תוצאה "
        "לבדיקת קבצים (0 מתוך 33,254 שורות), ולכן אי אפשר להפריד בין "
        "מי שעבר את הבדיקה למי שנכשל בה. «61% מגיעים לבדיקת קבצים» "
        "פירושו שבוצעה להם בדיקה, ולא שהם עברו אותה.\n\n"
